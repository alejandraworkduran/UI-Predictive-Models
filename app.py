...
def main():
    ...
    with col2:
        ...
        if st.button('Will Jellyfish and Humans be Present?'):
            ...
            st.session_state['dual_presence_prediction'] = model.predict(input_data)[0]
    ...
    with col4:
        ...
        if st.button('Predict Biovolume for Jellyfish'):
            if rank_species == '':
                st.session_state['biovolume_prediction'] = 'Please select a species.'
            else:
                ...
                st.session_state['biovolume_prediction'] = modelxgb.predict(input_data)[0]

    # Display results outside of columns
    if 'dual_presence_prediction' in st.session_state:
        prediction = st.session_state['dual_presence_prediction']
    ...
    if 'biovolume_prediction' in st.session_state:
        st.write('Biovolume Prediction in mm3:', st.session_state['biovolume_prediction'])
